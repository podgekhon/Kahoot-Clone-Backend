...
describe('test for adminUserDetails', () => {
  ...
  test('invalid token', () => {
    const updateResponse = requestAdminUserDetailsUpdate('abcd', 'newadmin@unsw.edu.au', 'Guanlin', 'Kong');
    expect(updateResponse.statusCode).toStrictEqual(401);
    expect(updateResponse.body).toStrictEqual({ error: expect.any(String) });
  });

  test('empty token', () => {
    const updateResponse = requestAdminUserDetailsUpdate(' ', 'newadmin@unsw.edu.au', 'Guanlin', 'Kong');
    expect(updateResponse.statusCode).toStrictEqual(401);
    expect(updateResponse.body).toStrictEqual({ error: expect.any(String) });
  });

  test('Update user details with invalid email', () => {
    const updateResponse = requestAdminUserDetailsUpdate(user1token, 'invalidemail', 'Guanlin', 'Kong');
    expect(updateResponse.statusCode).toStrictEqual(400);
    expect(updateResponse.body).toStrictEqual({ error: expect.any(String) });
  });

  test('Update user details with invalid first name', () => {
    const updateResponse = requestAdminUserDetailsUpdate(user1token, 'newadmin@unsw.edu.au', '1', 'Kong');
    expect(updateResponse.statusCode).toStrictEqual(400);
    expect(updateResponse.body).toStrictEqual({ error: expect.any(String) });
  });

  test('Update user details with invalid last name', () => {
    const updateResponse = requestAdminUserDetailsUpdate(user1token, 'newadmin@unsw.edu.au', 'Guanlin', '1');
    expect(updateResponse.statusCode).toStrictEqual(400);
    expect(updateResponse.body).toStrictEqual({ error: expect.any(String) });
  });
  ...
  test('Update user details successfully', () => {
    const updateResponse = requestAdminUserDetailsUpdate(user1token, 'newadmin@unsw.edu.au', 'Guanlin', 'Kong');
